  ...
  const handleNext = async () => {
    if (currentStep === 1) {
      ...
      if (ok) {
        ...
        try {
          const result = await sendEmailMutation.mutateAsync({ to: email });
          if (result.success) {
            router.push(`/verify-email?email=${encodeURIComponent(email)}`);
          } else {
            Alert.alert("Error", result.error || "Error sending verification code");
          }
        } catch {
          Alert.alert("Error", "Error sending verification code");
        }
      }
    }
  };
      ...
      <View style={{ marginVertical: 24 }}>
        ...
        <View style={{ marginTop: 24 }}>
          ...
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 16 }}>
            <View style={{
              width: 80, height: 80, backgroundColor: '#00AAEC', borderRadius: 40,
